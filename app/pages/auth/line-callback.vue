...
import { clearLineLoginAttempt, getLineCallbackAuthorization } from '../../utils/line-login'
...
onMounted(async () => {
  try {
    const authorization = getLineCallbackAuthorization(route.query, window.localStorage)
    const result = await callAppsScriptAction<LineLoginResult>(
      config.public.appsScriptUrl,
      'loginWithLine',
      { code: authorization.code, nonce: authorization.nonce },
    )
    clearLineLoginAttempt(window.localStorage)
    await handleLineLoginResult(result)
  } catch {
    clearLineLoginAttempt(window.localStorage)
    await navigateTo('/?login=retry', { replace: true })
  } finally {
...
<template>
  <main class="auth-page login-page">
    <section class="login-shell" aria-label="學員登入">
      ...
      <p v-if="isLoading || !bindingToken" class="login-status" role="status">
        {{ isLoading ? '正在確認身分...' : '正在返回登入頁...' }}
      </p>
      <form v-else aria-label="確認報名電話" @submit.prevent="bindPhone">
